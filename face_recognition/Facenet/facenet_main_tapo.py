...
try:
    ...
    try:
        ...
        print("[HELP] Make sure camera_config.py exists in face_recognition/Facenet/")
        sys.exit(1)

# Import authorization configuration
try:
    from face_recognition.Facenet import authorization_config
except ImportError:
    try:
        import authorization_config
    except ImportError:
        print("[WARN] authorization_config.py not found, using default access levels")
...
# -------------------- CONFIG --------------------
# Camera settings now managed by camera_config.py
# USE_WEBCAM = False  # DEPRECATED - use camera_config.CAMERA_MODE instead
# VIDEO_PATH = "..."  # DEPRECATED - use camera_config.py instead
...
# Authorization-based directories
if authorization_config:
    LOGS_AUTHORIZED_DIR = authorization_config.get_log_directory(authorization_config.AUTHORIZED)
    LOGS_PARTIAL_DIR = authorization_config.get_log_directory(authorization_config.PARTIAL)
    LOGS_UNAUTHORIZED_DIR = authorization_config.get_log_directory(authorization_config.UNAUTHORIZED)
    
    ANNOTATED_AUTHORIZED_DIR = authorization_config.get_annotated_directory(authorization_config.AUTHORIZED)
    ANNOTATED_PARTIAL_DIR = authorization_config.get_annotated_directory(authorization_config.PARTIAL)
    ANNOTATED_UNAUTHORIZED_DIR = authorization_config.get_annotated_directory(authorization_config.UNAUTHORIZED)
...
if SAVE_FACES:
    ...
    # Authorization-based directories
    if authorization_config:
        for p in [LOGS_AUTHORIZED_DIR, LOGS_PARTIAL_DIR, LOGS_UNAUTHORIZED_DIR,
                  ANNOTATED_AUTHORIZED_DIR, ANNOTATED_PARTIAL_DIR, ANNOTATED_UNAUTHORIZED_DIR]:
            os.makedirs(p, exist_ok=True)
            ...
            if results and results[0].boxes is not None:
                ...
                if boxes.id is not None:
                    ...
                    for track_id, bbox, conf in zip(track_ids, bboxes, confidences):
                        ...
                        # Get authorization level and display settings
                        if authorization_config:
                            auth_level = authorization_config.get_authorization_level(identity_name)
                            color = authorization_config.get_level_color(auth_level)
                            thickness = authorization_config.get_level_thickness(auth_level)
                            
                            # Brighter color if identity is locked
                            if is_locked and identity_name != 'Unknown':
                                # Boost brightness for locked identities
                        ...
                        # Enhanced label with authorization level, lock and pose information
                        pose_info = ""
                        ...
                        # Format label with authorization level
                        if authorization_config:
                            auth_level = authorization_config.get_authorization_level(identity_name)
                            level_prefix = authorization_config.get_level_prefix(auth_level)
                            label = f"{level_prefix} ID:{track_id} {identity_name} [{auth_level}]"
                        else:
...
def save_face_from_result(face_result, original_frame, name, frame_num, ts, known_last_saved):
    """Save detected face to disk, organized by authorization level"""
    try:
        # Check save interval
        ...
        if face_result['face_bbox'] is not None:
            ...
            if face_crop.size > 0:
                ...
                # Determine save directories based on authorization level
                if authorization_config:
                    auth_level = authorization_config.get_authorization_level(name)
                    logs_dir = authorization_config.get_log_directory(auth_level)
                    annotated_dir = authorization_config.get_annotated_directory(auth_level)
                else:
                    # Fallback to legacy directories
                    logs_dir = LOGS_KNOWN_DIR
                    annotated_dir = ANNOTATED_KNOWN_DIR
                ...
                # Save face crop
                face_path = os.path.join(logs_dir, filename)
                cv2.imwrite(face_path, face_crop)
                
                # Save annotated frame
                ann_path = os.path.join(annotated_dir, filename)
                cv2.imwrite(ann_path, original_frame)
                
                known_last_saved[name] = ts
                
                if authorization_config:
                    print(f"[INFO] Saved {auth_level} face: {filename}")
                else:
...
def grab_frames(cap, frame_q, stop_event):
    """
    Capture frames from VideoCapture with RTSP reconnection support
    """
    frame_num = 0
    ...
    try:
        while not stop_event.is_set():
            ...
            if not ret or frame is None:
                if camera_config.CAMERA_MODE == "rtsp" and camera_config.RTSP_AUTO_RECONNECT:
                    if reconnect_attempts < camera_config.RTSP_MAX_RECONNECT_ATTEMPTS:
                        reconnect_attempts += 1
                        print(f"[WARN] RTSP connection lost. Reconnecting... (Attempt {reconnect_attempts}/{camera_config.RTSP_MAX_RECONNECT_ATTEMPTS})")
                        
                        # Release and reinitialize
                        cap.release()
                        time.sleep(camera_config.RTSP_RECONNECT_DELAY)
                        ...
                        if cap.isOpened():
                            print("[INFO] RTSP reconnection successful")
                            reconnect_attempts = 0
                            continue
                        else:
                            print(f"[ERROR] RTSP reconnection failed (Attempt {reconnect_attempts})")
                            continue
                    else:
                        print("[ERROR] Max RTSP reconnection attempts reached. Stopping...")
                        stop_event.set()
                        break
                else:
                    # Not RTSP or auto-reconnect disabled
                    stop_event.set()
                    break
            
            # Successfully got frame - reset reconnect counter
            reconnect_attempts = 0
            ts = datetime.now()
...
def main():
    ...
    if authorization_config:
        authorization_config.print_authorization_summary()
    
    # Get camera source from configuration
    source = camera_config.get_camera_source()
    
    if source is None:
        print("[ERROR] Failed to get camera source from configuration")
        print("[HELP] Check camera_config.py settings")
        return
    ...
    # Apply RTSP-specific settings
    if camera_config.CAMERA_MODE == "rtsp":
        cap.set(cv2.CAP_PROP_BUFFERSIZE, camera_config.RTSP_BUFFER_SIZE)
        # Add timeout for RTSP
    ...
    if not cap.isOpened():
        ...
        print("[HELP] Run 'python face_recognition/Facenet/setup_camera.py' to test your camera")
        return
