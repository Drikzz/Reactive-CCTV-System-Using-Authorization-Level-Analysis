"""
Training script: ShuffleNetV2 feature extraction for behavior recognition

Features:
- ShuffleNetV2 (x1.0) pretrained on ImageNet, FROZEN BACKBONE (feature extraction)
...
Key difference from train_transfer_learning_shufflenetv2.py:
...
# Project root (two levels up since we're in shufflenetv2 subfolder)
...
def plot_and_save(history, save_dir, model_name):
    ...
    fig.suptitle('Training Curves - Feature Extraction (ShuffleNetV2)')
    plt.tight_layout()
    ...
    with open(summary_path, 'w') as f:
        f.write('='*60 + '\n')
        f.write('TRAINING SUMMARY - FEATURE EXTRACTION (ShuffleNetV2)\n')
        f.write('='*60 + '\n')
...
def train(args):
    ...
    print("\n🔒 Loading ShuffleNetV2 (x1.0) with FROZEN backbone...")
    model = models.shufflenet_v2_x1_0(weights=models.ShuffleNet_V2_X1_0_Weights.IMAGENET1K_V1)
    
    # FREEZE the backbone (all conv/stage layers before fc)
    # ShuffleNetV2 uses 'conv1', 'maxpool', 'stage2', 'stage3', 'stage4', 'conv5', 'fc'
    for name, param in model.named_parameters():
        if 'fc' not in name:  # Freeze everything except fc layer
            param.requires_grad = False
    
    # Replace fc layer with dropout for regularization
    in_features = model.fc.in_features
    model.fc = nn.Sequential(
        nn.Dropout(p=0.2),  # Add dropout to reduce overfitting
    ...
    print(f"✓ Only training fc layer\n")

    criterion = nn.CrossEntropyLoss()
    # Only optimize fc parameters (backbone is frozen)
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            checkpoint = {
                ...
                'history': history,
                'epoch': epoch+1,
                'model_type': 'shufflenet_v2_x1_0_feature_extraction',
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train ShuffleNetV2 (feature extraction - frozen backbone)')
    ...
    parser.add_argument('--save-dir', type=str, default='models/shufflenetv2', help='Directory to save model and plots')
    parser.add_argument('--model-name', type=str, default='shufflenetv2_feature_extraction.pth', help='Model filename')
...
# Notes:
# - Feature extraction mode: Backbone is FROZEN, only fc layer is trained
# - Much faster training and less prone to overfitting
# - Recommended for small datasets (< 500 images per class)
# - Uses dropout (p=0.2) in fc layer for regularization
...
# - For comparison with full fine-tuning, use train_transfer_learning_shufflenetv2.py
