"""
YOLOv8 + MobileNetV3-Small Integrated Tracking and Classification System
...
# Add parent directory to path for imports (now two levels up since we're in mobilenetv3-small subfolder)
BASE_DIR = Path(__file__).parent.parent.parent
sys.path.append(str(BASE_DIR))


class YOLOMobileNetV3SmallTracker:
    """
    Integrated YOLOv8 tracking + MobileNetV3-Small classification system.
    """
    ...
    def __init__(self, yolo_model_path, mobilenet_model_path, class_names=None, 
        ...
        Args:
            ...
            mobilenet_model_path (str): Path to trained MobileNetV3-Small checkpoint
        ...
        print(f"Loading MobileNetV3-Small model from: {mobilenet_model_path}")
        self.mobilenet, self.num_classes = self._load_mobilenet_v3_small(mobilenet_model_path)
        ...
        # Define transform for MobileNetV3-Small
        ...
        print(f"✓ MobileNetV3-Small Classes: {self.class_names}")
        print(f"✓ Device: {self.device}")
        print(f"✓ Initial Classification Interval: every {reclass_interval} frames")
        print(f"🧠 Adaptive Classification: ENABLED\n")
    
    def _load_mobilenet_v3_small(self, model_path):
        """Load MobileNetV3-Small model from checkpoint."""
    ...
    def _preprocess_crop(self, crop):
        """Preprocess a person crop for MobileNetV3-Small."""
        # Convert BGR to RGB
    ...
    def _classify_crop(self, crop):
        """
        Classify a person crop using MobileNetV3-Small.
    ...
    def process_video(self, video_path, display=True, save_output=None, 
        ...
        """
        Process video with YOLOv8 tracking + MobileNetV3-Small classification.
        ...
        try:
            while True:
                ...
                # Display
                if display:
                    cv2.imshow('YOLOv8 + MobileNetV3-Small Tracking', annotated_frame)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='YOLOv8 Tracking + MobileNetV3-Small Classification System'
    )
    ...
    parser.add_argument('--mobilenet-model', type=str,
                       default='models/mobilenetv3-small/mobilenet_v3_small_transfer.pth',
                       help='Path to MobileNetV3-Small model (default: models/mobilenetv3-small/mobilenet_v3_small_transfer.pth)')
    ...
    if not mobilenet_path.exists():
        print(f"Error: MobileNetV3-Small model not found: {mobilenet_path}")
        return
    
    # Initialize tracker
    print("\n" + "="*70)
    print("YOLOv8 + MOBILENETV3-SMALL INTEGRATED TRACKER")
    print("="*70 + "\n")
    
    tracker = YOLOMobileNetV3SmallTracker(
