...
class FrameClassifierConfig:
    ...
    aug_crop_scale: Tuple[float, float] = (0.91, 1.0)  # 0% min to 9% max zoom
    aug_crop_ratio: Tuple[float, float] = (0.95, 1.05)
    aug_brightness: float = 0.3  # ±30%
    aug_contrast: float = 0.2    # For exposure simulation
    aug_saturation: float = 0.0  # Not specified, disable
    aug_hue: float = 0.0         # Not specified, disable
    aug_blur_p: float = 0.3      # Probability to apply blur
    aug_blur_kernel: int = 3     # Up to 2px blur (kernel size 3)
    aug_noise_p: float = 0.3     # Probability to apply noise
    aug_noise_scale: float = 0.001  # Up to 0.1% of pixels
...
class FrameDataset(Dataset):
    ...
    def __init__(
        ...
        aug_flip_p: float = 0.5,
        aug_rotation: float = 15.0,
        aug_crop_scale: Tuple[float, float] = (0.91, 1.0),
        aug_crop_ratio: Tuple[float, float] = (0.95, 1.05),
        ...
        aug_noise_scale: float = 0.001,
    ):
        """
        Args:
            image_label_pairs: List of (image_path, label_idx) tuples
            classes: List of class names
            img_size: Target image size (square) - should match Roboflow preprocessing
        ...
        # Simple load and to tensor (no resizing - Roboflow already did that)
        ...
        # Training augmentation (matching Roboflow settings)
        if is_training:
            aug_transforms = []
            
            # Horizontal flip
            if aug_flip_p > 0:
                aug_transforms.append(T.RandomHorizontalFlip(p=aug_flip_p))
            
            # Rotation (-15° to +15°)
            if aug_rotation > 0:
                aug_transforms.append(T.RandomRotation(degrees=aug_rotation))
            
            # Crop (0% min to 9% max zoom)
            if aug_crop_scale[0] < 1.0:
                aug_transforms.append(T.RandomResizedCrop(
                    img_size,
                    scale=aug_crop_scale,
                    ratio=aug_crop_ratio
                ))
            
            # Color jitter for brightness and exposure (contrast)
            # Brightness: ±30%, Exposure (simulated as contrast): ±20%
            if aug_brightness > 0 or aug_contrast > 0 or aug_saturation > 0 or aug_hue > 0:
                aug_transforms.append(T.ColorJitter(
            ...
            # Gaussian blur (up to 2px)
            if aug_blur_p > 0 and aug_blur_kernel > 0:
                aug_transforms.append(T.RandomApply([
                    T.GaussianBlur(kernel_size=aug_blur_kernel, sigma=(0.1, 2.0))
                ], p=aug_blur_p))
            
            self.augmentation = T.Compose(aug_transforms) if aug_transforms else None
            
            # Store noise parameters (applied manually as torchvision doesn't have it)
            self.aug_noise_p = aug_noise_p
            self.aug_noise_scale = aug_noise_scale
        else:
            self.augmentation = None
            self.aug_noise_p = 0.0
    ...
    def _load_and_preprocess(self, image_path: str) -> torch.Tensor:
        ...
        # Load image (Roboflow already preprocessed, so just load and convert to tensor)
        try:
        ...
        # Convert to tensor (no resizing - Roboflow already did that)
    ...
    def __getitem__(self, idx: int) -> Tuple[torch.Tensor, int]:
        ...
        # Apply noise augmentation (up to 0.1% of pixels)
        if self.is_training and random.random() < self.aug_noise_p:
            noise = torch.randn_like(tensor) * self.aug_noise_scale
...
def build_dataloaders(cfg: FrameClassifierConfig):
    ...
    train_ds = FrameDataset(
        ...
        aug_hue=cfg.aug_hue,
        aug_blur_p=cfg.aug_blur_p,
        aug_blur_kernel=cfg.aug_blur_kernel,
        aug_noise_p=cfg.aug_noise_p,
        aug_noise_scale=cfg.aug_noise_scale,
    )
