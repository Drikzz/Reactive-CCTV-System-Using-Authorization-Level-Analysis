"""
Training script: MobileNetV2 feature extraction for behavior recognition

Features:
- MobileNetV2 pretrained on ImageNet, FROZEN BACKBONE (feature extraction)
...
Key difference from train_transfer_learning.py:
...
def plot_and_save(history, save_dir, model_name):
    ...
    fig.suptitle('Training Curves - Feature Extraction')
    plt.tight_layout()
    ...
    with open(summary_path, 'w') as f:
        f.write('='*60 + '\n')
        f.write('TRAINING SUMMARY - FEATURE EXTRACTION (MobileNetV2)\n')
        f.write('='*60 + '\n')
...
def train(args):
    ...
    print("\n🔒 Loading MobileNetV2 with FROZEN backbone...")
    ...
    for epoch in range(args.epochs):
        ...
        if val_acc > best_val_acc:
            ...
            checkpoint = {
                ...
                'history': history,
                'epoch': epoch+1,
                'model_type': 'mobilenet_v2_feature_extraction',
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Train MobileNetV2 (feature extraction - frozen backbone)')
    ...
    parser.add_argument('--model-name', type=str, default='mobilenet_feature_extraction.pth', help='Model filename')
...
# - For comparison with full fine-tuning, use train_transfer_learning.py
