"""
YOLOv8 + MobileNetV2 Integrated Tracking and Classification System

This script combines:
- YOLOv8 object detection and tracking (ByteTrack)
- MobileNetV2 behavior classification
...
# Add parent directory to path for imports (now two levels up since we're in MobileNetV2 subfolder)
BASE_DIR = Path(__file__).parent.parent.parent
sys.path.append(str(BASE_DIR))


class YOLOMobileNetTracker:
    """
    Integrated YOLOv8 tracking + MobileNetV2 classification system.
    """
    ...
    def __init__(self, yolo_model_path, mobilenet_model_path, class_names=None, 
        ...
        Args:
            ...
            mobilenet_model_path (str): Path to trained MobileNetV2 checkpoint
        ...
        print(f"Loading MobileNetV2 model from: {mobilenet_model_path}")
        self.mobilenet, self.num_classes = self._load_mobilenet(mobilenet_model_path)
        ...
        print(f"✓ MobileNetV2 Classes: {self.class_names}")
        print(f"✓ Device: {self.device}")
        print(f"✓ Initial Classification Interval: every {reclass_interval} frames")
        print(f"🧠 Adaptive Classification: ENABLED\n")
    
    def _load_mobilenet(self, model_path):
        """Load MobileNetV2 model from checkpoint."""
    ...
    def _preprocess_crop(self, crop):
        """Preprocess a person crop for MobileNetV2."""
        # Convert BGR to RGB
    ...
    def _classify_crop(self, crop):
        """
        Classify a person crop using MobileNetV2.
    ...
    def process_video(self, video_path, display=True, save_output=None, 
        ...
        """
        Process video with YOLOv8 tracking + MobileNetV2 classification.
        ...
        try:
            while True:
                ...
                # Display
                if display:
                    cv2.imshow('YOLOv8 + MobileNetV2 Tracking', annotated_frame)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='YOLOv8 Tracking + MobileNetV2 Classification System'
    )
    ...
    parser.add_argument('--mobilenet-model', type=str,
                       default='models/mobilenetv2/mobilenet_transfer.pth',
                       help='Path to MobileNetV2 model (default: models/mobilenetv2/mobilenet_transfer.pth)')
    ...
    # Initialize tracker
    print("\n" + "="*70)
    print("YOLOv8 + MOBILENETV2 INTEGRATED TRACKER")
    print("="*70 + "\n")
    
    tracker = YOLOMobileNetTracker(
